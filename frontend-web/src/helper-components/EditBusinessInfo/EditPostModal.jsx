...
import { editPost } from "@/apis/postsRequests";
...
const EditPostModal = ({
  ...
  async function handleDescriptionChange(event) {
    ...
    if (!newDesc || !hasMinLength(newDesc, 20)) {
      setError("Description can't be less than 20 digits");
      return;
    }

    try {
      const editPostData = await editPost(postID, newDesc, accessToken);
      if (editPostData.error) {
        console.error("Error editing post");
      } else if (editPostData.message) {
        setPostDescription(newDesc);
        hideModal();
      }
    } catch (error) {
  ...
  return (
    <div className="fixed top-0 right-0 w-screen h-screen bg-[#0000007f] z-50">
