...
// Dictionary to track socket connections
...
io.on("connection", (socket) => {
  ...
  socket.on("disconnect", () => {
    ...
    for (let key in socketConnections) {
      if (socketConnections[key] === socket.id) {
...
const getBusinesses = require("./Routes/getBusinesses");
...
app.use("/RateRelay", addPost(io, socketConnections));
app.use("/RateRelay", deletePost);
app.use("/RateRelay", getPost);
app.use("/RateRelay", addFeedback(io, socketConnections));
...
app.use("/RateRelay", addChatMessageRoute(io, socketConnections));
...
app.use("/RateRelay", getBusinesses);
