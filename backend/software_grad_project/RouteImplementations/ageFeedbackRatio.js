...
const ageFeedbackRatio = async (req, res) => {
  try {
    ...
    // Fetch total number of unique feedback entries for the specified business
    const totalFeedbackResult = await queryAsync(`
      SELECT COUNT(DISTINCT userName) AS totalFeedback
      FROM feedback
    ...
    const ageGroupsResult = await queryAsync(`
      ...
      FROM (SELECT DISTINCT userName, user_id FROM feedback WHERE businessName = ?) AS unique_feedback
      INNER JOIN user ON unique_feedback.user_id = user.userID`, [businessName]);
...
module.exports = ageFeedbackRatio;
